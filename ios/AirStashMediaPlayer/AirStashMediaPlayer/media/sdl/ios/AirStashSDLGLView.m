...
#import "AirStashSDLGLRender.h"
#import "AirStashSDLGLRenderI420.h"
#import "AirStashSDLGLRenderRV24.h"
#import "AirStashSDLGLRenderNV12.h"
#import "AirStashSDLGLRenderI444P10LE.h"
#include "sdl/sdl_timer.h"
#include "sdl/ios/sdl_ios.h"
#import "AirStashSDLHudViewController.h"

static NSString *const g_vertexShaderString = AIRSTASH_SHADER_STRING
(
...
@interface AirStashSDLGLView()
@property(atomic,strong) NSRecursiveLock *glActiveLock;
@property(atomic) BOOL glActivePaused;
@end

@implementation AirStashSDLGLView {
    ...
    id<AirStashSDLGLRender>        _renderer;
    ...
    AirStashSDLHudViewController *_hudViewController;
}
...
static int g_airstash_gles_queue_spec_key;
...
- (id) initWithFrame:(CGRect)frame useRenderQueue:(BOOL)useRenderQueue;
{
    ...
    if (self) {
        ...
        if (useRenderQueue) {
            ...
            _renderQueue = dispatch_queue_create("airstash-gles", attr);
            dispatch_queue_set_specific(_renderQueue,
                                        &g_airstash_gles_queue_spec_key,
                                        &g_airstash_gles_queue_spec_key,
                                        NULL);
        }
        ...
        _hudViewController = [[AirStashSDLHudViewController alloc] init];
...
- (BOOL)setupDisplay: (SDL_VoutOverlay *) overlay
{
    ...
    if (_renderer == nil) {
        if (overlay == nil) {
            ...
            _renderer = [[AirStashSDLGLRenderNV12 alloc] initWithTextureCache:_textureCache];
            _bytesPerPixel = 1;
            NSLog(@"OK use NV12 GL renderer");
        } else if (overlay->format == SDL_FCC_I420) {
            _frameChroma = overlay->format;
            _renderer = [[AirStashSDLGLRenderI420 alloc] init];
            _bytesPerPixel = 1;
            NSLog(@"OK use I420 GL renderer");
        } else if (overlay->format == SDL_FCC_RV24) {
            _frameChroma = overlay->format;
            _renderer = [[AirStashSDLGLRenderRV24 alloc] init];
            _bytesPerPixel = 3;
            NSLog(@"OK use RV24 GL renderer");
        } else if (overlay->format == SDL_FCC_I444P10LE) {
            _frameChroma = overlay->format;
            _renderer = [[AirStashSDLGLRenderI444P10LE alloc] init];
            _bytesPerPixel = 2;
...
{
    if (self->_useRenderQueue && !dispatch_get_specific(&g_airstash_gles_queue_spec_key)) {
    ...
    if ([self setupGLOnce]) {
        ...
        if (![self tryLockGLActive]) {
            if (0 == (_tryLockErrorCount % 100)) {
                NSLog(@"AirStashSDLGLView:display: unable to tryLock GL active: %d\n", _tryLockErrorCount);
            }
            _tryLockErrorCount++;
            return;
        }
        ...
        if (!_didStopGL) {
            if (_context == nil) {
                NSLog(@"AirStashSDLGLView: nil EAGLContext\n");
                return;
            }
...
- (void)displayInternal: (SDL_VoutOverlay *) overlay
{
    ...
    if (![self setupDisplay:overlay]) {
        NSLog(@"AirStashSDLGLView: setupDisplay failed\n");
        return;
    }
...
{
    NSLog(@"AirStashSDLGLView:applicationWillEnterForeground: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:NO];
}

- (void)applicationDidBecomeActive
{
    NSLog(@"AirStashSDLGLView:applicationDidBecomeActive: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:NO];
}

- (void)applicationWillResignActive
{
    NSLog(@"AirStashSDLGLView:applicationWillResignActive: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:YES];
}

- (void)applicationDidEnterBackground
{
    NSLog(@"AirStashSDLGLView:applicationDidEnterBackground: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:YES];
}

- (void)applicationWillTerminate
{
    NSLog(@"AirStashSDLGLView:applicationWillTerminate: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:YES];
}
...
#pragma mark AirStashFFHudController
