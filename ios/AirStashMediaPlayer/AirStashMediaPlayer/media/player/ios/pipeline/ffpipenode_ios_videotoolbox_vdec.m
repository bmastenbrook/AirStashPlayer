...
#include "player/ff_ffpipeline.h"
...
{
    AirStashFF_Pipenode_Opaque* opaque = (AirStashFF_Pipenode_Opaque*) arg;
...
AirStashFF_Pipenode *ffpipenode_create_video_decoder_from_ios_videotoolbox(FFPlayer *ffp)
{
    if (!ffp || !ffp->is)
        return NULL;
    ...
    if (!node)
        return node;
    memset(node, sizeof(AirStashFF_Pipenode), 0);
