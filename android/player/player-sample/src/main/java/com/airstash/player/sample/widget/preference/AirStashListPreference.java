...
package com.airstash.player.sample.widget.preference;
...
public class AirStashListPreference extends ListPreference {
    private CharSequence[] mEntrySummaries;

    public AirStashListPreference(Context context) {
        super(context);
        initPreference(context, null);
    }

    public AirStashListPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
        initPreference(context, attrs);
    }

    public AirStashListPreference(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initPreference(context, attrs);
    }

    public AirStashListPreference(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private final void initPreference(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.AirStashListPreference, 0, 0);
        if (a == null)
            return;

        mEntrySummaries = a
                .getTextArray(R.styleable.AirStashListPreference_entrySummaries);
