...
import com.airstash.player.player.annotations.AccessedByNative;
import com.airstash.player.player.annotations.CalledByNative;
import com.airstash.player.player.misc.IMediaDataSource;
import com.airstash.player.player.misc.ITrackInfo;
import com.airstash.player.player.misc.AirStashTrackInfo;
import com.airstash.player.player.pragma.DebugLog;


public final class AirStashMediaPlayer extends AbstractMediaPlayer {
    private final static String TAG = AirStashMediaPlayer.class.getName();
    ...
    //----------------------------------------
    // options
    public static final int AIRSTASH_LOG_UNKNOWN = 0;
    public static final int AIRSTASH_LOG_DEFAULT = 1;

    public static final int AIRSTASH_LOG_VERBOSE = 2;
    public static final int AIRSTASH_LOG_DEBUG = 3;
    public static final int AIRSTASH_LOG_INFO = 4;
    public static final int AIRSTASH_LOG_WARN = 5;
    public static final int AIRSTASH_LOG_ERROR = 6;
    public static final int AIRSTASH_LOG_FATAL = 7;
    public static final int AIRSTASH_LOG_SILENT = 8;
     ...
     */
    private static final AirStashLibLoader sLocalLibLoader = new AirStashLibLoader() {
        @Override
    ...
    public static void loadLibrariesOnce(AirStashLibLoader libLoader) {
        synchronized (AirStashMediaPlayer.class) {
            if (!mIsLibLoaded) {
                if (libLoader == null)
                    libLoader = sLocalLibLoader;

                libLoader.loadLibrary("ffmpeg");
                libLoader.loadLibrary("sdl");
     ...
     * synchronously instantiating a AirStashMediaPlayer from a Uri or resource.
     * <p>
     * When done with the AirStashMediaPlayer, you should call {@link #release()}, to
     * free the resources. If not released, too many AirStashMediaPlayer instances
     * may result in an exception.
     * </p>
     */
    public AirStashMediaPlayer() {
     ...
     */
    public AirStashMediaPlayer(AirStashLibLoader libLoader) {
        initPlayer(libLoader);
    }

    private void initPlayer(AirStashLibLoader libLoader) {
         ...
         */
        native_setup(new WeakReference<AirStashMediaPlayer>(this));
    }

    /*
     * Update the AirStashMediaPlayer SurfaceTexture. Call after setting a new
     * display surface.
     */
    ...
    public void setWakeMode(Context context, int mode) {
        ...
        mWakeLock = pm.newWakeLock(mode | PowerManager.ON_AFTER_RELEASE,
                AirStashMediaPlayer.class.getName());
    ...
    @Override
    public AirStashTrackInfo[] getTrackInfo() {
        Bundle bundle = getMediaMeta();
        if (bundle == null)
            return null;

        AirStashMediaMeta mediaMeta = AirStashMediaMeta.parse(bundle);
        if (mediaMeta == null || mediaMeta.mStreams == null)
            return null;

        ArrayList<AirStashTrackInfo> trackInfos = new ArrayList<AirStashTrackInfo>();
        for (AirStashMediaMeta.AirStashStreamMeta streamMeta: mediaMeta.mStreams) {
            AirStashTrackInfo trackInfo = new AirStashTrackInfo(streamMeta);
            if (streamMeta.mType.equalsIgnoreCase(AirStashMediaMeta.AIRSTASHM_VAL_TYPE__VIDEO)) {
                trackInfo.setTrackType(ITrackInfo.MEDIA_TRACK_TYPE_VIDEO);
            } else if (streamMeta.mType.equalsIgnoreCase(AirStashMediaMeta.AIRSTASHM_VAL_TYPE__AUDIO)) {
                trackInfo.setTrackType(ITrackInfo.MEDIA_TRACK_TYPE_AUDIO);
            }
            trackInfos.add(trackInfo);
        }

        return trackInfos.toArray(new AirStashTrackInfo[trackInfos.size()]);
    }
    ...
    /**
     * Releases resources associated with this AirStashMediaPlayer object. It is
     * considered good practice to call this method when you're done using the
     * AirStashMediaPlayer. In particular, whenever an Activity of an application is
     * paused (its onPause() method is called), or stopped (its onStop() method
     * is called), this method should be invoked to release the AirStashMediaPlayer
     ...
     * if a AirStashMediaPlayer object is no longer needed may also lead to
    ...
    public MediaInfo getMediaInfo() {
        ...
        mediaInfo.mMediaPlayerName = "airstashplayer";
        ...
        try {
            mediaInfo.mMeta = AirStashMediaMeta.parse(_getMediaMeta());
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return mediaInfo;
    ...
    private native void native_setup(Object AirStashMediaPlayer_this);

    private native void native_finalize();

    private native void native_message_loop(Object AirStashMediaPlayer_this);
    ...
    private static class EventHandler extends Handler {
        private final WeakReference<AirStashMediaPlayer> mWeakPlayer;

        public EventHandler(AirStashMediaPlayer mp, Looper looper) {
            super(looper);
            mWeakPlayer = new WeakReference<AirStashMediaPlayer>(mp);
        }

        @Override
        public void handleMessage(Message msg) {
            AirStashMediaPlayer player = mWeakPlayer.get();
            if (player == null || player.mNativeMediaPlayer == 0) {
                DebugLog.w(TAG,
                        "AirStashMediaPlayer went away with unhandled events");
                return;
            }
     ...
     * thread. We use a weak reference to the original AirStashMediaPlayer object so
    ...
    private static void postEventFromNative(Object weakThiz, int what,
        ...
        AirStashMediaPlayer mp = (AirStashMediaPlayer) ((WeakReference) weakThiz).get();
        if (mp == null) {
            return;
        }
    ...
    private static boolean onNativeInvoke(Object weakThiz, int what, Bundle args) {
        ...
        WeakReference<AirStashMediaPlayer> weakPlayer = (WeakReference<AirStashMediaPlayer>) weakThiz;
        AirStashMediaPlayer player = weakPlayer.get();
        if (player == null)
    ...
    public static class DefaultMediaCodecSelector implements OnMediaCodecSelectListener {
        ...
        public String onMediaCodecSelect(IMediaPlayer mp, String mimeType, int profile, int level) {
            ...
            ArrayList<AirStashMediaCodecInfo> candidateCodecList = new ArrayList<AirStashMediaCodecInfo>();
            ...
            for (int i = 0; i < numCodecs; i++) {
                ...
                for(String type: types) {
                    ...
                    AirStashMediaCodecInfo candidate = AirStashMediaCodecInfo.setupCandidate(codecInfo, mimeType);
                    if (candidate == null)
                        continue;
            ...
            AirStashMediaCodecInfo bestCodec = candidateCodecList.get(0);

            for (AirStashMediaCodecInfo codec : candidateCodecList) {
                if (codec.mRank > bestCodec.mRank) {
                    bestCodec = codec;
                }
            }

            if (bestCodec.mRank < AirStashMediaCodecInfo.RANK_LAST_CHANCE) {
