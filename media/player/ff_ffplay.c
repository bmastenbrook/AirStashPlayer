...
#endif
    airstashmeta_set_avformat_context_l(ffp->meta, ic);
    ffp->stat.bit_rate = ic->bit_rate;
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0)
        airstashmeta_set_int64_l(ffp->meta, AIRSTASHM_KEY_VIDEO_STREAM, st_index[AVMEDIA_TYPE_VIDEO]);
    if (st_index[AVMEDIA_TYPE_AUDIO] >= 0)
        airstashmeta_set_int64_l(ffp->meta, AIRSTASHM_KEY_AUDIO_STREAM, st_index[AVMEDIA_TYPE_AUDIO]);
...
/*        if (ffp->infinite_buffer<1 && !is->seek_req &&
#ifdef FFP_MERGE
              (is->audioq.size + is->videoq.size + is->subtitleq.size > MAX_QUEUE_SIZE || (drainQueue && is->audioq.size + is->videoq.size + is->subtitleq.size > MIN_QUEUE_SIZE))
#else
    ...
    if (ffp->infinite_buffer<1 && !is->seek_req &&
        ((is->audioq.size + is->videoq.size > MAX_QUEUE_SIZE || (drainQueue && is->audioq.size + is->videoq.size > MIN_QUEUE_SIZE))
...
inline static int log_level_av_to_airstash(int av_level)
{
    int airstash_level = AIRSTASH_LOG_VERBOSE;
    if      (av_level <= AV_LOG_PANIC)      airstash_level = AIRSTASH_LOG_FATAL;
    else if (av_level <= AV_LOG_FATAL)      airstash_level = AIRSTASH_LOG_FATAL;
    else if (av_level <= AV_LOG_ERROR)      airstash_level = AIRSTASH_LOG_ERROR;
    else if (av_level <= AV_LOG_WARNING)    airstash_level = AIRSTASH_LOG_WARN;
    else if (av_level <= AV_LOG_INFO)       airstash_level = AIRSTASH_LOG_INFO;
    // AV_LOG_VERBOSE means detailed info
    else if (av_level <= AV_LOG_VERBOSE)    airstash_level = AIRSTASH_LOG_INFO;
    else if (av_level <= AV_LOG_DEBUG)      airstash_level = AIRSTASH_LOG_DEBUG;
    else if (av_level <= AV_LOG_TRACE)      airstash_level = AIRSTASH_LOG_VERBOSE;
    else                                    airstash_level = AIRSTASH_LOG_VERBOSE;
    return airstash_level;
}

inline static int log_level_airstash_to_av(int airstash_level)
{
    int av_level = AIRSTASH_LOG_VERBOSE;
    if      (airstash_level >= AIRSTASH_LOG_SILENT)   av_level = AV_LOG_QUIET;
    else if (airstash_level >= AIRSTASH_LOG_FATAL)    av_level = AV_LOG_FATAL;
    else if (airstash_level >= AIRSTASH_LOG_ERROR)    av_level = AV_LOG_ERROR;
    else if (airstash_level >= AIRSTASH_LOG_WARN)     av_level = AV_LOG_WARNING;
    else if (airstash_level >= AIRSTASH_LOG_INFO)     av_level = AV_LOG_INFO;
    // AV_LOG_VERBOSE means detailed info
    else if (airstash_level >= AIRSTASH_LOG_DEBUG)    av_level = AV_LOG_DEBUG;
    else if (airstash_level >= AIRSTASH_LOG_VERBOSE)  av_level = AV_LOG_TRACE;
    else if (airstash_level >= AIRSTASH_LOG_DEFAULT)  av_level = AV_LOG_TRACE;
    else if (airstash_level >= AIRSTASH_LOG_UNKNOWN)  av_level = AV_LOG_TRACE;
...
static void ffp_log_callback_brief(void *ptr, int level, const char *fmt, va_list vl)
{
    ...
    int ffplv __unused = log_level_av_to_airstash(level);
    VLOG(ffplv, AIRSTASH_LOG_TAG, fmt, vl);
}
...
static void ffp_log_callback_report(void *ptr, int level, const char *fmt, va_list vl)
{
    ...
    ALOG(ffplv, AIRSTASH_LOG_TAG, "%s", line);
}
...
{
    int av_level = log_level_airstash_to_av(log_level);
    av_log_set_level(av_level);
}

void ffp_global_set_inject_callback(airstash_inject_callback cb)
{
    airstashav_register_inject_callback(cb);
}

void ffp_io_stat_register(void (*cb)(const char *url, int type, int bytes))
{
    // avairstash_io_stat_register(cb);
}
...
{
    // avairstash_io_stat_complete_register(cb);
}
...
void ffp_destroy(FFPlayer *ffp)
{
    ...
    airstashmeta_destroy_p(&ffp->meta);
    ffp_reset_internal(ffp);
...
static void ffp_show_version_int(FFPlayer *ffp, const char *module, unsigned version)
{
    av_log(ffp, AV_LOG_INFO, "%-*s: %u.%u.%u\n",
           ...
           (unsigned int)AIRSTASHVERSION_GET_MAJOR(version),
           (unsigned int)AIRSTASHVERSION_GET_MINOR(version),
           (unsigned int)AIRSTASHVERSION_GET_MICRO(version));
}
...
int ffp_prepare_async_l(FFPlayer *ffp, const char *file_name)
{
    ...
    if (strlen(file_name) + 1 > 1024) {
        ...
        if (avio_find_protocol_name("airstashlongurl:")) {
            av_dict_set(&ffp->format_opts, "airstashlongurl-url", file_name, 0);
            file_name = "airstashlongurl:";
        }
    }
...
void ffp_check_buffering_l(FFPlayer *ffp)
{
    ...
    if (hwm_in_ms > 0) {
        ...
        if (video_cached_duration > 0 && audio_cached_duration > 0) {
            cached_duration_in_ms = (int)AIRSTASHMIN(video_cached_duration, audio_cached_duration);
...
AirStashMediaMeta *ffp_get_meta_l(FFPlayer *ffp)
{
    if (!ffp)
        return NULL;
