...
typedef struct AirStashFF_Pipeline_Opaque {
    FFPlayer      *ffp;
    SDL_mutex     *surface_mutex;
    jobject        jsurface;
    volatile bool  is_surface_need_reconfigure;

    bool         (*mediacodec_select_callback)(void *opaque, airstashmp_mediacodecinfo_context *mcc);
...
inline static bool check_ffpipeline(AirStashFF_Pipeline* pipeline, const char *func_name)
{
