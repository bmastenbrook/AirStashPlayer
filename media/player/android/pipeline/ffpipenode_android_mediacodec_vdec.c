...
#include "sdl/android/sdl_codec_android_mediaformat_java.h"
#include "sdl/android/sdl_codec_android_mediacodec_java.h"
#include "sdl/android/sdl_codec_android_mediacodec_dummy.h"
#include "sdl/android/sdl_vout_android_nativewindow.h"
#include "sdl/android/sdl_vout_overlay_android_mediacodec.h"
#include "player/ff_ffpipenode.h"
#include "player/ff_ffplay.h"
#include "player/ff_ffplay_debug.h"
#include "h264_nal.h"
#include "hevc_nal.h"
...
typedef struct AirStashFF_Pipenode_Opaque {
    FFPlayer                 *ffp;
    AirStashFF_Pipeline           *pipeline;
    Decoder                  *decoder;
    SDL_Vout                 *weak_vout;

    airstashmp_mediacodecinfo_context mcc;
...
} AirStashFF_Pipenode_Opaque;

static SDL_AMediaCodec *create_codec_l(JNIEnv *env, AirStashFF_Pipenode *node)
{
    AirStashFF_Pipenode_Opaque        *opaque   = node->opaque;
    airstashmp_mediacodecinfo_context *mcc      = &opaque->mcc;
...
static int reconfigure_codec_l(JNIEnv *env, AirStashFF_Pipenode *node, jobject new_surface)
{
    AirStashFF_Pipenode_Opaque *opaque   = node->opaque;
    int                    ret      = 0;
    ...
    if (new_surface) {
        ...
        if (AirStash_ExceptionCheck__catchAll(env) || !opaque->jsurface)
            goto fail;
    } else {
...
#if 0
static int reconfigure_codec(JNIEnv *env, AirStashFF_Pipenode *node)
{
    AirStashFF_Pipenode_Opaque *opaque = node->opaque;
...
static int feed_input_buffer(JNIEnv *env, AirStashFF_Pipenode *node, int64_t timeUs, int *enqueue_count)
{
    AirStashFF_Pipenode_Opaque *opaque   = node->opaque;
    FFPlayer              *ffp      = opaque->ffp;
    AirStashFF_Pipeline        *pipeline = opaque->pipeline;
    VideoState            *is       = ffp->is;
    Decoder               *d        = &is->viddec;
    ...
    if (d->pkt_temp.data) {
        ...
        if (ffpipeline_is_surface_need_reconfigure_l(pipeline)) {
            ...
            if (opaque->jsurface == new_surface ||
                ...
                AirStash_DeleteGlobalRef__p(env, &new_surface);
            } else {
...
static double pts_from_buffer_info(AirStashFF_Pipenode *node, SDL_AMediaCodecBufferInfo *buffer_info)
{
    AirStashFF_Pipenode_Opaque *opaque     = node->opaque;
    FFPlayer              *ffp        = opaque->ffp;
    VideoState            *is         = ffp->is;
...
static int drain_output_buffer_l(JNIEnv *env, AirStashFF_Pipenode *node, int64_t timeUs, int *dequeue_count, AVFrame *frame, int *got_frame)
{
    AirStashFF_Pipenode_Opaque *opaque   = node->opaque;
    FFPlayer              *ffp      = opaque->ffp;
    int                    ret      = 0;
...
static int drain_output_buffer(JNIEnv *env, AirStashFF_Pipenode *node, int64_t timeUs, int *dequeue_count, AVFrame *frame, int *got_frame)
{
    AirStashFF_Pipenode_Opaque *opaque = node->opaque;
    SDL_LockMutex(opaque->acodec_mutex);
...
static int func_flush(AirStashFF_Pipenode *node)
{
    AirStashFF_Pipenode_Opaque *opaque   = node->opaque;
...
AirStashFF_Pipenode *ffpipenode_create_video_decoder_from_android_mediacodec(FFPlayer *ffp, AirStashFF_Pipeline *pipeline, SDL_Vout *vout)
{
    ALOGD("ffpipenode_create_video_decoder_from_android_mediacodec()\n");
    if (SDL_Android_GetApiLevel() < AIRSTASH_API_16_JELLY_BEAN)
        return NULL;
    ...
    if (ffp->mediacodec_auto_rotate &&
        rotate_degrees != 0 &&
        SDL_Android_GetApiLevel() >= AIRSTASH_API_21_LOLLIPOP) {
    ...
    AirStash_DeleteGlobalRef__p(env, &jsurface);
    if (ret != 0)
        goto fail;
