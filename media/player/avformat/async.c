...
static void call_inject(URLContext *h)
{
    ...
    if (opaque && inject_callback) {
        AirStashAVInject_AsyncStatistic stat;
        ...
        inject_callback(opaque, AIRSTASHAVINJECT_ASYNC_STATISTIC, &stat, sizeof(stat));
    }
}
...
static const AVOption options[] = {
    ...
    { "async-forwards-capacity",    "max bytes that may be read forward in background",
        OFFSET(forwards_capacity),  AIRSTASHAV_OPTION_INT64(128 * 1024, 128 * 1024, 128 * 1024 * 1024) },
    { "async-backwards-capacity",   "max bytes that may be seek backward without seeking in inner protocol",
        OFFSET(backwards_capacity), AIRSTASHAV_OPTION_INT64(128 * 1024, 128 * 1024, 128 * 1024 * 1024) },
    {NULL},
};
