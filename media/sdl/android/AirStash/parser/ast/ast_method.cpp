...
void Method::build_c_member_id_load(std::ostream &os)
{
    ...
    if (annotation) {
        os << build_indent() << "if (AirStash_GetSystemAndroidApiLevel(env) >= " << annotation->get_value() << ") {" << std::endl;
    ...
    os << build_indent() << get_c_jni_id() << " = " << (is_static() ? "AirStash_GetStaticMethodID__catchAll" : "AirStash_GetMethodID__catchAll")
...
void Method::_build_c_func_string_argument_cast_statements(std::ostream &os, int flags)
{
    ...
    for (NULL; begin != end; ++begin) {
        ...
        if (flags & FLAG_CATCH_ALL) {
            os << "    if (AirStash_ExceptionCheck__catchAll(env) || !" << name << ")" << std::endl;
        } else {
            os << "    if (AirStash_ExceptionCheck__throwAny(env) || !" << name << ")" << std::endl;
        }
...
void Method::_build_c_func_string_argument_release_statements(std::ostream &os, int flags)
{
    ...
    for (NULL; begin != end; ++begin) {
        ...
        os << "    AirStash_DeleteLocalRef__p(env, &" << name << ");" << std::endl;
    }
}
...
void Method::_build_c_func_impl_void_type_statement(std::ostream &os, int flags)
{
    ...
    if (flags & FLAG_WITH_C_STRING) {
        ...
        os << "    AirStash_ExceptionCheck__catchAll(env);" << std::endl;
    } else {
...
void Method::_build_c_func_impl_basic_type_statement(std::ostream &os, int flags)
{
    ...
    if (flags & FLAG_WITH_C_STRING) {
        ...
        if (flags & FLAG_CATCH_ALL) {
            os << "    if (AirStash_ExceptionCheck__catchAll(env)) {" << std::endl;
        } else {
            os << "    if (AirStash_ExceptionCheck__throwAny(env)) {" << std::endl;
        }
...
void Method::_build_c_func_impl_reference_type_statement(std::ostream &os, int flags)
{
    ...
    if (flags & FLAG_AS_C_BUFFER) {
        ...
        if (flags & FLAG_CATCH_ALL) {
            os << "    if (AirStash_ExceptionCheck__catchAll(env) || !local_string) {" << std::endl;
        } else {
            os << "    if (AirStash_ExceptionCheck__throwAny(env) || !local_string) {" << std::endl;
        }
        os << "        goto fail;" << std::endl;
        os << "    }" << std::endl;
        os << std::endl;
        ...
        if (flags & FLAG_CATCH_ALL) {
            os << "    if (AirStash_ExceptionCheck__catchAll(env) || !c_str) {" << std::endl;
        } else {
            os << "    if (AirStash_ExceptionCheck__throwAny(env) || !c_str) {" << std::endl;
        }
        os << "        goto fail;" << std::endl;
        os << "    }" << std::endl;
        os << std::endl;
        ...
        os << std::endl;
        os << "fail:" << std::endl;
        os << "    AirStash_ReleaseStringUTFChars__p(env, local_string, &c_str);" << std::endl;
        os << "    AirStash_DeleteLocalRef__p(env, &local_string);" << std::endl;
        os << "    return ret_value;" << std::endl;
    } else if (flags & FLAG_AS_GLOBAL_REF) {
        assert(get_type()->is_reference_type());
        ...
        if (flags & FLAG_CATCH_ALL) {
            os << "    if (AirStash_ExceptionCheck__catchAll(env) || !local_object) {" << std::endl;
        } else {
            os << "    if (AirStash_ExceptionCheck__throwAny(env) || !local_object) {" << std::endl;
        }
        os << "        ret_object = " << get_type()->get_c_default_value() << ";" << std::endl;
        os << "        goto fail;" << std::endl;
        os << "    }" << std::endl;
        os << std::endl;
        os << "    ret_object = AirStash_NewGlobalRef__catchAll(env, local_object);\n";
        ...
        os << std::endl;
        os << "fail:" << std::endl;
        os << "    AirStash_DeleteLocalRef__p(env, &local_object);\n";
        ...
        if (flags & FLAG_CATCH_ALL) {
            os << "    if (AirStash_ExceptionCheck__catchAll(env) || !ret_object) {" << std::endl;
        } else {
            os << "    if (AirStash_ExceptionCheck__throwAny(env) || !ret_object) {" << std::endl;
        }
        os << "        ret_object = " << get_type()->get_c_default_value() << ";" << std::endl;
        os << "        goto fail;" << std::endl;
        os << "    }" << std::endl;
