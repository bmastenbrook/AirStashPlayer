...
int audiotrack_get_native_output_sample_rate(JNIEnv *env)
{
    ...
    jint retval = AirStashC_AudioTrack__getNativeOutputSampleRate(env, STREAM_MUSIC);
    if (AirStash_ExceptionCheck__catchAll(env) || retval <= 0)
        return -1;
...
{
    AirStashC_AudioTrack__setStereoVolume__catchAll(env, atrack->thiz, left_volume, right_volume);
}
...
SDL_Android_AudioTrack *SDL_Android_AudioTrack_new_from_spec(JNIEnv *env, SDL_Android_AudioTrack_Spec *spec)
{
    ...
    case ENCODING_PCM_FLOAT:
        ...
        if (sdk_int < AIRSTASH_API_21_LOLLIPOP) {
    ...
    int min_buffer_size = AirStashC_AudioTrack__getMinBufferSize(env,
        atrack->spec.sample_rate_in_hz,
        atrack->spec.channel_config,
        atrack->spec.audio_format);
    if (AirStash_ExceptionCheck__catchAll(env) || min_buffer_size <= 0) {
        ALOGE("%s: AirStashC_AudioTrack__getMinBufferSize: return %d:", __func__, min_buffer_size);
        free(atrack);
        return NULL;
    }
    ...
    atrack->thiz = AirStashC_AudioTrack__AudioTrack__asGlobalRef__catchAll(env,
    ...
    // atrack->max_volume = AirStashC_AudioTrack__getMaxVolume__catchAll(env);
    // atrack->min_volume = AirStashC_AudioTrack__getMinVolume__catchAll(env);
    atrack->max_volume = 0.0f;
    atrack->min_volume = 1.0f;

    // extra init
    float init_volume = 1.0f;
    init_volume = AIRSTASHMIN(init_volume, atrack->max_volume);
    init_volume = AIRSTASHMAX(init_volume, atrack->min_volume);
    ALOGI("%s: init volume as %f/(%f,%f)", __func__, init_volume, atrack->min_volume, atrack->max_volume);
    AirStashC_AudioTrack__setStereoVolume__catchAll(env, atrack->thiz, init_volume, init_volume);
...
{
    AirStash_DeleteGlobalRef__p(env, &atrack->byte_buffer);
    atrack->byte_buffer_capacity = 0;

    if (atrack->thiz) {
        AirStashC_AudioTrack__release(env, atrack->thiz);
        AirStash_DeleteGlobalRef__p(env, &atrack->thiz);
    }
...
int SDL_Android_AudioTrack_reserve_byte_buffer(JNIEnv *env, SDL_Android_AudioTrack *atrack, int size_in_byte)
{
    ...
    int capacity = AIRSTASHMAX(size_in_byte, atrack->min_buffer_size);
    atrack->byte_buffer = AirStash_NewByteArray__asGlobalRef__catchAll(env, capacity);
...
int SDL_Android_AudioTrack_write(JNIEnv *env, SDL_Android_AudioTrack *atrack, uint8_t *data, int size_in_byte)
{
    ...
    int retval = AirStashC_AudioTrack__write(env, atrack->thiz, atrack->byte_buffer, 0, (int)size_in_byte);
    if (AirStash_ExceptionCheck__catchAll(env))
        return -1;
...
{
    SDLTRACE("%s", __func__);
    int audioSessionId = AirStashC_AudioTrack__getAudioSessionId(env, atrack->thiz);
    if (AirStash_ExceptionCheck__catchAll(env))
        return 0;
...
{
    AirStashC_AudioTrack__setSpeed(env, atrack->thiz, speed);
    if (AirStash_ExceptionCheck__catchAll(env))
        return;
