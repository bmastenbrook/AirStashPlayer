...
#include "sdl_codec_android_mediaformat_internal.h"
...
static bool SDL_AMediaFormatJava_getInt32(SDL_AMediaFormat* aformat, const char* name, int32_t *out)
{
    ...
    jint ret = AirStashC_MediaFormat__getInteger__withCString(env, android_media_format, name);
...
static void SDL_AMediaFormatJava_setInt32(SDL_AMediaFormat* aformat, const char* name, int32_t value)
{
    ...
    AirStashC_MediaFormat__setInteger__withCString(env, android_media_format, name, value);
    if (AirStash_ExceptionCheck__catchAll(env)) {
...
static void SDL_AMediaFormatJava_setBuffer(SDL_AMediaFormat* aformat, const char* name, void* data, size_t size)
{
    ...
    if (!opaque->android_byte_buffer) {
        opaque->android_byte_buffer = AirStashC_ByteBuffer__allocateDirect__asGlobalRef__catchAll(env, size);
        if (!opaque->android_byte_buffer) {
    ...
    ret = AirStashC_ByteBuffer__assignData__catchAll(env, opaque->android_byte_buffer, data, size);
    if (ret < 0) {
        AIRSTASH_FUNC_FAIL_TRACE();
        return;
    }

    AirStashC_MediaFormat__setByteBuffer__withCString(env, android_media_format, name, opaque->android_byte_buffer);
    if (AirStash_ExceptionCheck__catchAll(env)) {
...
SDL_AMediaFormat *SDL_AMediaFormatJava_init(JNIEnv *env, jobject android_format)
{
    ...
    if (AirStash_ExceptionCheck__catchAll(env) || !global_android_media_format) {
        return NULL;
    }
...
SDL_AMediaFormat *SDL_AMediaFormatJava_createVideoFormat(JNIEnv *env, const char *mime, int width, int height)
{
    ...
    jobject android_media_format = AirStashC_MediaFormat__createVideoFormat__withCString__asGlobalRef__catchAll(env, mime, width, height);
    if (AirStash_ExceptionCheck__catchAll(env) || !android_media_format) {
        return NULL;
    }
