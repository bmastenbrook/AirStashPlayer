...
static int android_render_yv12_on_yv12(ANativeWindow_Buffer *out_buffer, const SDL_VoutOverlay *overlay)
{
    ...
    int min_height = AIRSTASHMIN(out_buffer->height, overlay->h);
    int dst_y_stride = out_buffer->stride;
    int dst_c_stride = AIRSTASHALIGN(out_buffer->stride / 2, 16);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (dst_line_size == src_line_size) {
            ...
            // TODO: 9 padding
            int bytewidth = AIRSTASHMIN(dst_line_size, src_line_size);
...
int SDL_Android_NativeWindow_display_l(ANativeWindow *native_window, SDL_VoutOverlay *overlay)
{
    ...
    int buff_w = AIRSTASHALIGN(overlay->w, 2);
    int buff_h = AIRSTASHALIGN(overlay->h, 2);
