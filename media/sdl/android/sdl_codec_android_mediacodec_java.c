...
SDL_AMediaFormat *SDL_AMediaCodecJava_getOutputFormat(SDL_AMediaCodec *thiz)
{
    ...
    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *)thiz->opaque;
    jobject local_android_format = AirStashC_MediaCodec__getOutputFormat__catchAll(env, opaque->android_media_codec);
...
static sdl_amedia_status_t SDL_AMediaCodecJava_delete(SDL_AMediaCodec* acodec)
{
    ...
    if (opaque) {
        if (opaque->android_media_codec) {
            AirStashC_MediaCodec__release__catchAll(env, opaque->android_media_codec);
        }
    ...
    uint32_t flags)
{
    ...
    AirStashC_MediaCodec__configure(env, android_media_codec, android_media_format, android_surface, crypto, flags);
...
static sdl_amedia_status_t SDL_AMediaCodecJava_start(SDL_AMediaCodec* acodec)
{
    ...
    AirStashC_MediaCodec__start(env, android_media_codec);
    if (AirStash_ExceptionCheck__catchAll(env)) {
...
static sdl_amedia_status_t SDL_AMediaCodecJava_stop(SDL_AMediaCodec* acodec)
{
    ...
    jobject android_media_codec = SDL_AMediaCodecJava_getObject(env, acodec);
    AirStashC_MediaCodec__stop(env, android_media_codec);
    if (AirStash_ExceptionCheck__catchAll(env)) {
...
static sdl_amedia_status_t SDL_AMediaCodecJava_flush(SDL_AMediaCodec* acodec)
{
    ...
    jobject android_media_codec = SDL_AMediaCodecJava_getObject(env, acodec);
    AirStashC_MediaCodec__flush(env, android_media_codec);
    if (AirStash_ExceptionCheck__catchAll(env)) {
...
static ssize_t SDL_AMediaCodecJava_writeInputData(SDL_AMediaCodec* acodec, size_t idx, const uint8_t *data, size_t size)
{
    ...
    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *)acodec->opaque;
    input_buffer_array = AirStashC_MediaCodec__getInputBuffers__catchAll(env, opaque->android_media_codec);
    if (!input_buffer_array)
        return -1;

    int buffer_count = (*env)->GetArrayLength(env, input_buffer_array);
    if (AirStash_ExceptionCheck__catchAll(env) || idx < 0 || idx >= buffer_count) {
        ALOGE("%s: idx(%d) < count(%d)\n", __func__, (int)idx, (int)buffer_count);
        goto fail;
    }

    input_buffer = (*env)->GetObjectArrayElement(env, input_buffer_array, idx);
    if (AirStash_ExceptionCheck__catchAll(env) || !input_buffer) {
...
ssize_t SDL_AMediaCodecJava_dequeueInputBuffer(SDL_AMediaCodec* acodec, int64_t timeoutUs)
{
    ...
    jobject android_media_codec = opaque->android_media_codec;
    jint idx = AirStashC_MediaCodec__dequeueInputBuffer(env, android_media_codec, (jlong)timeoutUs);
    if (AirStash_ExceptionCheck__catchAll(env)) {
...
sdl_amedia_status_t SDL_AMediaCodecJava_queueInputBuffer(SDL_AMediaCodec* acodec, size_t idx, off_t offset, size_t size, uint64_t time, uint32_t flags)
{
    ...
    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *)acodec->opaque;
    jobject android_media_codec = opaque->android_media_codec;
    AirStashC_MediaCodec__queueInputBuffer(env, android_media_codec, (jint)idx, (jint)offset, (jint)size, (jlong)time, (jint)flags);
    if (AirStash_ExceptionCheck__catchAll(env)) {
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }
...
ssize_t SDL_AMediaCodecJava_dequeueOutputBuffer(SDL_AMediaCodec* acodec, SDL_AMediaCodecBufferInfo *info, int64_t timeoutUs)
{
    ...
    if (!opaque->output_buffer_info) {
        opaque->output_buffer_info = AirStashC_MediaCodec__BufferInfo__BufferInfo__asGlobalRef__catchAll(env);
        if (!opaque->output_buffer_info)
            return AMEDIACODEC__UNKNOWN_ERROR;
    }
    ...
    while (1) {
        idx = AirStashC_MediaCodec__dequeueOutputBuffer(env, android_media_codec, opaque->output_buffer_info, (jlong)timeoutUs);
        if (AirStash_ExceptionCheck__catchAll(env)) {
            ALOGI("%s: Exception\n", __func__);
            return AMEDIACODEC__UNKNOWN_ERROR;
        }
        if (idx == AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED) {
            ...
            if (info) {
                info->offset              = AirStashC_MediaCodec__BufferInfo__offset__get__catchAll(env, opaque->output_buffer_info);
                info->size                = AirStashC_MediaCodec__BufferInfo__size__get__catchAll(env, opaque->output_buffer_info);
                info->presentationTimeUs  = AirStashC_MediaCodec__BufferInfo__presentationTimeUs__get__catchAll(env, opaque->output_buffer_info);
                info->flags               = AirStashC_MediaCodec__BufferInfo__flags__get__catchAll(env, opaque->output_buffer_info);
            }
        }
        break;
    }
...
sdl_amedia_status_t SDL_AMediaCodecJava_releaseOutputBuffer(SDL_AMediaCodec* acodec, size_t idx, bool render)
{
    ...
    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *)acodec->opaque;
    jobject android_media_codec = opaque->android_media_codec;
    AirStashC_MediaCodec__releaseOutputBuffer(env, android_media_codec, (jint)idx, (jboolean)render);
    if (AirStash_ExceptionCheck__catchAll(env)) {
...
static SDL_AMediaCodec* SDL_AMediaCodecJava_init(JNIEnv *env, jobject android_media_codec)
{
    ...
    if (AirStash_ExceptionCheck__catchAll(env) || !global_android_media_codec) {
        return NULL;
    }
...
SDL_AMediaCodec* SDL_AMediaCodecJava_createByCodecName(JNIEnv *env, const char *codec_name)
{
    ...
    jobject android_media_codec = AirStashC_MediaCodec__createByCodecName__withCString(env, codec_name);
    if (AirStash_ExceptionCheck__catchAll(env) || !android_media_codec) {
        return NULL;
    }
